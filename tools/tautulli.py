"""Tautulli API wrapper for Apollo Bot tool calls."""
...
async def _get(cmd: str, params: dict | None = None) -> dict:
    """Make a GET request to the Tautulli API."""
    url = f"{Config.TAUTULLI_URL}/api/v2"
    base_params = {"apikey": Config.TAUTULLI_API_KEY, "cmd": cmd}
    if params:
