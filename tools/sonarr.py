"""Sonarr API wrapper for Apollo Bot tool calls."""
...
async def _get(endpoint: str, params: dict | None = None) -> dict | list:
    url = f"{Config.SONARR_URL}/api/v3{endpoint}"
...
async def get_queue() -> str:
    ...
    if not records:
        return "The Sonarr download queue is empty — no episodes are currently downloading."
...
async def lookup_series(title: str) -> str:
    """Look up a TV series in Sonarr's library."""
    results = await _get("/series/lookup", params={"term": title})

    if not results:
        return f"No series found matching '{title}' in Sonarr."
    ...
    return "**Sonarr Lookup:**\n" + "\n".join(lines)


async def get_system_status() -> str:
    """Get Sonarr system health status."""
