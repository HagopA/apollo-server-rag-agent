...
from tools import seerr, radarr, sonarr, tautulli
...
TOOLS = [
    {
        "name": "search_media",
        "description": "Search for movies or TV shows in Seerr to see if they exist and their request status.",
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {
    ...
    {
        "name": "get_requests",
        "description": "Get recent media requests from Seerr. Can filter by status.",
        "input_schema": {
            "type": "object",
            "properties": {
                "status": {
    ...
    },
    {
        ...
        "description": "Check the Radarr movie download queue to see what movies are currently downloading.",
        "input_schema": {"type": "object", "properties": {}},
    },
    {
        "name": "get_tv_queue",
        "description": "Check the Sonarr TV show download queue to see what episodes are currently downloading.",
        "input_schema": {"type": "object", "properties": {}},
    },
    {
        "name": "lookup_movie",
        "description": "Look up a movie in Radarr to see if it's downloaded and what quality it's in.",
        "input_schema": {
            "type": "object",
            "properties": {
                "title": {
    ...
    {
        "name": "lookup_series",
        "description": "Look up a TV series in Sonarr to see how many episodes are downloaded.",
        "input_schema": {
            "type": "object",
            "properties": {
                "title": {
...
TOOL_HANDLERS = {
    "search_media": lambda args: seerr.search_media(args["query"]),
    "get_requests": lambda args: seerr.get_requests(
        status=args.get("status", "all"), count=args.get("count", 10)
    ),
    "get_request_status": lambda args: seerr.get_request_by_title(args["title"]),
    "get_movie_queue": lambda args: radarr.get_queue(),
    "get_tv_queue": lambda args: sonarr.get_queue(),
    "lookup_movie": lambda args: radarr.lookup_movie(args["title"]),
    "lookup_series": lambda args: sonarr.lookup_series(args["title"]),
    "get_plex_activity": lambda args: tautulli.get_activity(),
    "get_recently_added": lambda args: tautulli.get_recently_added(args.get("count", 5)),
}
