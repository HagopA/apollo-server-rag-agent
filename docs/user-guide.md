...
**Step 2 — Request Portal Login (via Plex):** After passing the security gate, you'll land on the media request service login page. Click "Sign in with Plex" and complete the Plex sign-in if prompted. This links your session to your Plex profile so the system knows which requests belong to you.

After both steps, you'll be taken directly into the request portal where you can search for and request content.
...
1. **Request submitted** — Your request is instantly approved and passed to the right download manager based on content type
2. **Routed by content type** — Movie requests go to the movie management service. TV show and anime requests go to the TV management service
3. **Best release searched** — The search indexer queries multiple release sources to find the best quality version available. Some sources are protected by anti-bot systems, and the challenge solver silently handles those challenges so the search runs smoothly
4. **Download begins** — The best matching release is sent to the download client, which retrieves the file
...
All content is downloaded in the best available quality thanks to the quality profile manager, which keeps quality standards consistent across the server:
...
You don't need to do anything differently to request anime. Just search for it in the request portal the same way you would a TV show, and the system routes it to the right place automatically. If you want quick access, pin the Anime library in your Plex app.
