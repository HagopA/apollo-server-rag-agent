...
- **The download is stalled** — Occasionally a download can stall with no connections. The queue cleanup service monitors for this and will automatically remove stalled downloads and trigger a new search. This can cause a brief delay but is self-correcting.
...
### I see "Available" in the request portal but it's not in Plex
...
### The request portal loads but shows an error
